...
function getCellInnerHtml(table_name, display_name, tnStr, sessionVar, searchVar_str) {
	...
	innerStr = innerStr + ' onclick="javascript:doTheOrderBy(\'' + table_name + '\', \'' + searchVar_str + '\', \'' + tnStr + '\')"> <b> ' + display_name;
...
function printHeader(row, isAp, searchVar, colsInfo, tabAps, tabCcs) { // , apOrCcHeader, isAp, epiHeader, locHeader, isolnHeader, isPriv){
	...
	while (i < colsInfo.length) {
		...
		// Ccs table 3
		else if (i < colsInfo.length - 1 && colsInfo[i + 1].table_name.match(/\_merge$/) && colsInfo[i].table_name.match(/3_[0-9]+$/)) {


			// add empty column
			if (isEmptyAdded2 == false) {
				var cell = row.insertCell(dispColCounter++);
				cell.innerHTML = "";
				isEmptyAdded2 = true;
			}
...
function doCcLayout(isolates, searchVar, isShowCol, colsInfo, tabAps, tabCcs, serverStatusChoices, assignStatusChoices, privStatusChoices, org) {
	...
	for (var i = 0; i < isolates.length; i++) {
		...
		dict_colors = printTheIsoRow(row, isolates[i], colsInfo, serverStatusChoices, assignStatusChoices, privStatusChoices, isolateId.db_col, false, isShowCol, false, dict_colors, org);
	}
}
...
function printTheIsoRow(row, isolate, colsInfo, serverStatusChoices, assignStatusChoices, privStatusChoices, isolateIdCol, isAp, isShowCol, isShowDst, dict_colors, org) {
	console.log('The row is ', row);
	console.log('The isolate is ', isolate);
	...
	while (i < colsInfo.length) {
		...
		// Isolate, project, aps, ccs
			...
			if (colsInfo[i].table_name.toLowerCase() == 'identifier' && isIsoIdentifierPrint == false) {
				var cell = row.insertCell(dispColCounter++);
				cell.innerHTML = "<a class=\"speLink\" href=\"/" + org + "/isolate-" + isolate[colsInfo[isolateIdCol].db_col] + "-detail\">" + isolate[colsInfo[i].db_col] + "</a>";
...
function doApLayout(isolates, searchVar, isShowCol, isShowDst, colsInfo, tabAps, tabCcs, serverStatusChoices, assignStatusChoices, privStatusChoices, org) {
	...
	for (var i = 0; i < isolates.length; i++) {
		...
		dict_colors = printTheIsoRow(row, isolates[i], colsInfo, serverStatusChoices, assignStatusChoices, privStatusChoices, isolateId.db_col, true, isShowCol, isShowDst, dict_colors, org);
