...
import json
import importlib
from ..FuncsAuxAndDb import getHeaders, routeToRightRawQFn, getPoolOfCcMergeIds, mergeCcOdc
...
from ..FuncsAuxAndDb import queryDb
...
def page(request, org):
	...
	if request.user.is_authenticated:
		...
		(isoCount, isolates, dict_pageInfo, dict_mergedIds, list_colsInfo, list_tabAps, list_tabCcs, list_serverStatus, list_assignStatus, list_privStatus, boolChoices) = routeToRightRawQFn.getIsolatesFromRightFn(arr_ap, arr_cc, arr_epi, arr_loc, arr_isln, arr_iso, pageNumToGet, maxIsolatesPerPage, None, False, orderBy, dir, isCsv, isMgt9Ap, searchType, isMr, org)
		isolates = mergeCcOdc.get_merges(list_colsInfo, isolates, org)
	...
	if isMr:
		...
		return render(request, 'Templates/isolateTable.html', {"isolates": jsonIso, "isoCount": isoCount, "pageInfo": dict_pageInfo, "isAp": isAp, 'isDst': isDst, 'isMgtColor': isMgtColor, "colsInfo": list_colsInfo, 'tabAps': list_tabAps, 'tabCcs': list_tabCcs, 'serverStatus': list_serverStatus, 'assignStatus': list_assignStatus, 'privStatus': list_privStatus, "mergedIds": mergedIds, "searchType": searchType, "boolChoices": boolChoices, "organism": org, 'apDownloadLvls': settings.AP_DWN_LVLS_DISPLAY_NAME[org]})
