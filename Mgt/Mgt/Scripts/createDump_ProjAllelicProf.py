...
def getAndPrint(appClass, queryNumLimit, tns_mgt9, dir_filesForDownload, appName, aScheme):
    ...
    for projObj in projObjs:
        ...
        if numPubIso > 0:
            ...
            # print(str(numPubIso) + "\t" + str(totalNumQuerys))
            ...
            # Each query
            startCnt = 0; endCount=queryNumLimit
            for queryNum in range(0, totalNumQuerys):
                ...
                # print (str(queryNum) + "\t" + str(startCnt) + "\t" + str(endCount-1))

                

                mgt9Ids = appClass.models.Isolate.objects.filter(project_id=projObj.id, assignment_status='A', **{'mgt__' + zeroTn+"__isnull": False}).order_by('id').values_list('mgt__' + zeroTn, flat=True)[startCnt:endCount]
...
def mvFileToRightDirAndDelOld(filename, dir_filesForDownload, toDelPattern):
    ...
    apFiles = glob.glob(dir_filesForDownload + toDelPattern + "*")
    for apFn in apFiles:
